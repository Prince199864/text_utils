...
function TextForm(props) {
    ...
    return (
        <div className="container" style={props.mode === 'light' ? { backgroundColor: '#accbed', color: 'black' } : { backgroundColor: "black", color: 'white' }}>
            ...
            <button disabled={text.length === 0} type="button" className={`btn btn-${props.mode === 'light' ? 'success' : 'secondary'} btn-sm mx-2 my-2`} onClick={toggleUpCase}>Convert To UpperCase</button>
            <button disabled={text.length === 0} type="button" className={`btn btn-${props.mode === 'light' ? 'success' : 'secondary'} btn-sm mx-2 my-2`} onClick={toggleLoCase}>Convert To LowerCase</button>
            <button disabled={text.length === 0} type="button" className={`btn btn-${props.mode === 'light' ? 'success' : 'secondary'} btn-sm mx-2 my-2`} onClick={toggleExSpaces}>Remove Extra Spaces</button>
            <button disabled={text.length === 0} type="button" className={`btn btn-${props.mode === 'light' ? 'success' : 'secondary'} btn-sm mx-2 my-2`} onClick={handleCopy}>Copy to Clipboard</button>
            <button disabled={text.length === 0} type="button" className={`btn btn-${props.mode === 'light' ? 'success' : 'secondary'} btn-sm mx-2 my-2`} onClick={handleClear}>Clear Text</button>
            <h3>Text Summary</h3>
